...
export class GpuDesignerNode extends DesignerNode {
	...
	public _init() {
		...
		//初始化时候创建一个FrameBuffer
		//调用父类的createTexture创建一个空的texture
	...
	buildShader(source: string) {
		...
		//这里的porcess函数是什么？之后继承节点中实现对tex_coord的处理
        ...
        `;
		//继承节点调用buildShader构造各自的process以及InitRandom函数重新加入第0行
		fragSource =
			fragSource +
